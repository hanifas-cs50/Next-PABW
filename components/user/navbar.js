...
export default function UserNavbar({ user, stranger, settings }) {
  const [drop, setDrop] = useState(false);

  return (
    <nav className="w-full max-h-14 h-full grid grid-cols-3 items-center border-b-2">
      <div className="flex justify-start">
        {stranger ? (
          <>
            <Link href={{ pathname: "/home/profile" }}>
              <button className="h-full w-13 grid place-items-center rounded-r cursor-pointer">
                <svg
            ...
            <span className="font-medium">{stranger?.email}</span>
          </>
        ) : (
          <span className="px-4 font-medium">{user?.email}</span>
        )}
      </div>
